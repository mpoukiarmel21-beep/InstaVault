...
/// Per-container app language + region/country spoofing. When a container pins an
/// app language and/or a region, this makes the process answer that identity on
/// every locale surface Instagram reads:
///
...
///   - NSTimeZone.systemTimeZone / localTimeZone / defaultTimeZone (region to tz).
///   - CFLocaleCopyCurrent / CFTimeZoneCopySystem (fishhook, for C-level callers
///     in the app binary).
///
/// Gated on ACTIVE isolation: never touches locale state on the real account, and
/// is a no-op when the container sets neither language nor region.
///
...
/// given container. No-op when it sets no language/region. Run once at launch,
/// AFTER isolation is confirmed active.
...
/// AUTO-DETECT: returns a language code and region detected from the device's
/// system settings, matched against the supported languages/regions. Useful for
/// seeding a new container with the user's current locale. Returns nil when no
/// supported language/region matches the device.
